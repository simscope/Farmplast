...
export const FIRMWARE_BUCKET='chiller-firmware'
export const SIGNED_URL_SECONDS=1800
const devices=['ESP32-CH2-PLC','ESP32-CH3-PLC']
...
export async function inspectFirmware(file,target) {
  ...
  if(file.size<256 || file.size>OTA_SLOT_BYTES) throw new Error('Image exceeds the OTA slot or is too small.')
  const bytes=await file.arrayBuffer(), view=new DataView(bytes)
  if(bytes.byteLength!==file.size || view.getUint8(0)!==0xe9 || view.getUint16(12,true)!==0 || view.getUint32(32,true)!==0xabcd5432)
    throw new Error('Expected a classic ESP32 application image, not a merged image.')
  ...
  const device=uniqueMarker(text,'CH23OTA_DEVICE','[A-Za-z0-9_-]{1,48}')
  const version=uniqueMarker(text,'CH23OTA_VERSION','[A-Za-z0-9._-]{1,48}')
...
export async function uploadAndApprove(client,file,target,expected) {
  ...
  const status=await rpc(client,'chiller_firmware_status',{p_device:target})
  ...
  return rpc(client,'chiller_firmware_publish',{p_device:target,p_version:image.version,p_sha256:image.sha256,p_size:image.size,p_storage_path:image.storage_path,p_verified_sha256:image.sha256})
}
...
export async function queueFirmware(client,{device,code,release,requestId}) {
  const {grant}=await rpc(client,'chiller_firmware_unlock',{p_device:device,p_code:code})
  ...
  return rpc(client,'chiller_ota_queue',{p_device:device,p_grant:grant,p_id:requestId,p_release:release.id,p_signed_url:data.signedUrl,p_signed_url_expires_at:signedUrlExpiry(data.signedUrl)})
}
