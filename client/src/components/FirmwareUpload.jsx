...
import {inspectFirmware,uploadAndApprove} from '../utils/chillerFirmware'

export default function FirmwareUpload({device,onClose,onApproved,client=supabase}) {
  ...
  async function select(event) {
    ...
    catch {if(selection.current===generation) setError('Invalid firmware. Check the target, markers, classic ESP32 image and OTA slot size.')}
  ...
  return <div role="dialog" aria-modal="true" aria-labelledby="firmware-upload-title" className="fixed inset-0 z-[2100] grid place-items-center bg-slate-950/90 p-4">
    <section className="max-h-[calc(100dvh-32px)] w-full max-w-xl overflow-y-auto rounded-2xl border border-slate-500 bg-slate-900 p-6 text-white">
      ...
      {image&&<>
        ...
        <ul className="mb-4 grid grid-cols-2 gap-2 text-emerald-300">{['Device marker','Version marker','Image size','Target','Classic ESP32'].map(v=><li key={v}>✓ {v}</li>)}</ul>
      </>}
