export default function PlantSummary({ plant }) {
  const items = [
    ['Controllers', plant.controllers?.length ?? '—'],
  ...
  return <section aria-label="Plant Summary" className="pa-summary">
    {items.map(([label, value]) => <div className="pa-summary-item" key={label}><span>{label}</span><strong>{value}</strong><small>{label === 'Climate Zones' ? plant.climateSummary ? 'Reported zone health' : plant.climateZones.some(zone => zone.connection !== 'NOT CONFIGURED') ? 'No data' : 'Not configured' : label === 'Controllers' && plant.controllers ? plant.cabinetConnection : label === 'Mixers' && plant.mixerSummary ? 'Reported feedback' : typeof value !== 'number' ? 'Not configured' : label === 'Active Alarms' ? 'Reported alarms' : 'Planned inventory'}</small></div>)}
  </section>
}
