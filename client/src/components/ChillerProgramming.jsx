...
export default function ChillerProgramming({deviceCode,label,data,error,onRefresh}) {
  ...
  return <div>
    <div style={{display:'flex',gap:16,flexWrap:'wrap',alignItems:'center'}}>
      <div>Firmware: <strong>{data?.device?.version||'Device not OTA registered'}</strong> {active && <>→ {latest.release?.version||selected?.version||'selected version'}</>}<br />Available: {versions[0]?.version||'No approved update'}<br />Last seen: {timestamp(data?.device?.last_seen)} · {online?'ONLINE':'OFFLINE'}</div>
