...
test('gateway OTA has no second polling task or output interlock',async()=>{
  ...
  assert.doesNotMatch(source,/xTaskCreate|vTaskDelay|relayWrite|updateOutputsLogic|otaFreshControls|ch1Maintenance/)
  ...
  assert.match(sketch,/resolution=merge-duplicates,return=minimal/)
