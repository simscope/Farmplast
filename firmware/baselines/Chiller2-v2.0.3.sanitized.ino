/*
  WT32-ETH01 - CHILLER 2
  Minimal telemetry + network recovery + Internet OTA hook
  Firmware version: 2.0.1

  Architecture:
    Ethernet (LAN8720) -> local PLC Modbus TCP only
    Wi-Fi              -> Supabase / Internet only

  IMPORTANT:
    - Ethernet has NO default gateway on purpose.
    - Wi-Fi is forced as the Internet/default interface on Arduino-ESP32 3.x.
    - Only values used by the current Chiller 2 HMI are posted to Supabase.
    - No full 40011..40061 dump is sent anymore.

  HMI register map used:
    R40011 bit 0   = Heartbeat
    R40011 bit 14  = System Running

    R40012 bit 8   = Comp 1A Enabled
    R40012 bit 9   = Comp 1B Enabled
    R40012 bit 10  = Comp 1C Enabled

    R40013 bit 8   = Comp 2A Enabled
    R40013 bit 9   = Comp 2B Enabled
    R40013 bit 10  = Comp 2C Enabled

    R40023 / 10    = Process Setpoint F
    R40024 / 10    = Entering Fluid F
    R40025 / 10    = Leaving Fluid F

    R40051         = Flow C1 GPM
    R40052         = Flow C2 GPM
    R40053         = Capacity C1 Tons
    R40054         = Capacity C2 Tons
    R40056         = Evap Out C1 F
    R40057         = Evap Out C2 F
    R40060 / 10    = Process Delta T F
    R40061         = System Demand %

  Supabase writes:
    Selected CH2_Rxxxxx points above
    CH2_HEARTBEAT
    CH2_SYSTEM_RUNNING
    CH2_COMP_1A_ENABLED
    CH2_COMP_1B_ENABLED
  ...
  Internet OTA:
    The firmware checks this fixed Edge Function URL every 30 minutes:
      /functions/v1/ch2-ota

    Expected server behavior:
      204 or 304 = no update
      200 + firmware binary = install update
      x-SHA256 response header is strongly recommended

    Request headers include:
      x-device-code
      x-device-secret
      x-ESP32-version (added by HTTPUpdate)

  NOTE:
    OTA TLS currently uses setInsecure() to stay compatible with the existing
    Supabase connection. Before relying on unattended Internet OTA, the next
    backend step should add proper CA validation / signed firmware policy.
*/
...
// FIRMWARE
// ======================================================
static const char* FIRMWARE_VERSION = "2.0.3";
...
static const char* OTA_URL =
  "https://eeobivvwjzakbweluwtm.supabase.co/functions/v1/ch2-ota";

static const unsigned long OTA_FIRST_CHECK_DELAY_MS = 120000UL;       // 2 min after boot
static const unsigned long OTA_CHECK_INTERVAL_MS    = 1800000UL;      // 30 min
...
static IPAddress eth_local_ip(192, 168, 1, 51);
...
// New firmware: POST every 15 sec with 20 readings max.
...
bool pollChiller() {
  ...
  ch.heartbeat     = ((status[0] >> 0)  & 0x01) != 0;
  ...
  ch.deltaTF        = ch.r40060 / 10.0f;

  Serial.printf(
    "[CH2] SP=%.1f IN=%.1f OUT=%.1f DT=%.1f "
    "F1=%u F2=%u C1=%u C2=%u DEM=%u RUN=%u HB=%u\n",
    ch.setpointF,
    ch.enteringFluidF,
    ch.leavingFluidF,
    ch.deltaTF,
...
String buildRpcBody() {
  ...
  if (ch.valid) {
    ...
    appendUIntReading(body, first, "CH2_R40053", ch.r40053);
    appendUIntReading(body, first, "CH2_R40054", ch.r40054);

    appendUIntReading(body, first, "CH2_R40056", ch.r40056);
    appendUIntReading(body, first, "CH2_R40057", ch.r40057);

    appendUIntReading(body, first, "CH2_R40060", ch.r40060);
    appendUIntReading(body, first, "CH2_R40061", ch.r40061);

    // Only booleans visible on HMI.
    appendBoolReading(body, first, "CH2_HEARTBEAT", ch.heartbeat);
  ...
  appendBoolReading(body, first, "CH2_ONLINE", chillerOnline);
...
void checkInternetOta() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[OTA] Skip: Wi-Fi offline");
    return;
  }

  forceInternetToWiFi();

  Serial.print("[OTA] Checking, current=");
  Serial.println(FIRMWARE_VERSION);

  WiFiClientSecure client;

  // Compatibility with current firmware network setup.
  // NEXT HARDENING STEP: replace with CA validation before unattended OTA use.
  client.setInsecure();
  ...
  t_httpUpdate_return ret = httpUpdate.update(
    ...
    FIRMWARE_VERSION,
    [](HTTPClient* otaHttp) {
      otaHttp->addHeader("apikey", SUPABASE_ANON_KEY);
      otaHttp->addHeader(
        "Authorization",
        "Bearer " + String(SUPABASE_ANON_KEY)
      );
      otaHttp->addHeader("x-device-code", DEVICE_CODE);
      otaHttp->addHeader("x-device-secret", DEVICE_SECRET);
  ...
  switch (ret) {
    ...
    case HTTP_UPDATE_OK:
      // Usually not reached when rebootOnUpdate(true).
...
void serviceOta() {
  ...
  if (!otaFirstCheckDone) {
    if (now - bootMs < OTA_FIRST_CHECK_DELAY_MS) {
      return;
    }

    otaFirstCheckDone = true;
    lastOtaCheckMs = now;
    checkInternetOta();
    return;
  }
